...
def order_customized_detail(request, public_id):
    ...
    if payment_result == "success" and session_id and settings.STRIPE_SECRET_KEY:
        try:
            ...
            pass
    elif payment_result == "success" and payment_intent_id and settings.STRIPE_SECRET_KEY:
        try:
            stripe.api_key = settings.STRIPE_SECRET_KEY
            payment_intent = stripe.PaymentIntent.retrieve(payment_intent_id)
            if (payment_intent.get("metadata") or {}).get("public_id") == str(obj.public_id):
                _confirm_customized_payment_intent(payment_intent)
                obj.refresh_from_db()
        except stripe.error.StripeError:
            # The signed webhook remains the authoritative fallback.
            pass
...
def order_customized_pay(request, public_id):
    ...
    try:
        payment_intent = stripe.PaymentIntent.create(
            ...
            description=f"Kişiye Özel Nomaya Deneyimi • {obj.location}",
        )
        if bool(payment_intent.livemode) != (publishable_mode == "live"):
            return JsonResponse(
                {"error": "Stripe ödemesi ile yayınlanabilir anahtar aynı modda değil."},
                status=503,
            )
    except stripe.error.StripeError:
        return JsonResponse(
    ...
    send_telegram_message(
        f"<b>💳 Stripe Ödeme Penceresi Açıldı</b>\n\n"
        f"<b>ID:</b> {tg(obj.id)}\n"
        f"<b>Konum:</b> {tg(obj.location)}\n"
        f"<b>Tutar:</b> ${tg(obj.total_price)} USD\n"
        f"<b>Payment Intent:</b> {tg(payment_intent.id)}"
    )

    return JsonResponse({"client_secret": payment_intent.client_secret})


def _confirm_customized_payment_intent(payment_intent):
    """Mark a successful Stripe PaymentIntent paid exactly once."""
    metadata = payment_intent.get("metadata") or {}
    request_id = metadata.get("customized_request_id")
    if not request_id or payment_intent.get("status") != "succeeded":
        return False
    ...
    with transaction.atomic():
        ...
        if (
            ...
            or payment_intent.get("amount_received") != expected_amount
        ):
            return False
        ...
        transaction.on_commit(lambda: send_telegram_message(
            ...
            f"<b>Gün:</b> {tg(obj.days)}\n"
            f"<b>Tutar:</b> ${tg(obj.total_price)} USD\n"
            f"<b>Payment Intent:</b> {tg(payment_intent.get('id'))}"
        ))
    return True
...
def stripe_webhook(request):
    ...
    if event["type"] == "checkout.session.completed":
        ...
        if (payment_intent.get("metadata") or {}).get("customized_request_id"):
            if not _confirm_customized_payment_intent(payment_intent):
                return HttpResponse("Payment details did not match.", status=400)
